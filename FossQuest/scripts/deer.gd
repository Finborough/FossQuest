...
func _on_nav_timer_timeout() -> void:
		...
		var raw_target_pos = global_position + (dir_away * 300)
		var safe_target_pos = NavigationServer2D.map_get_closest_point(agent.get_navigation_map(), raw_target_pos)
		agent.target_position = safe_target_pos
